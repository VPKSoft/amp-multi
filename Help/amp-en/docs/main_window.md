...
#### Stash current queue
Saves to current queue into the stash clearing the current queue in the process. The function can be used to listen something else in between the current queue playback. Keyboard shortcut: <kbd>Ctrl</kbd>+<kbd>T</kbd>.

#### Pop stashed queue
Restores the stashed queue overriding the current queue in the process. Keyboard shortcut: <kbd>Ctrl</kbd>+<kbd>P</kbd>.
